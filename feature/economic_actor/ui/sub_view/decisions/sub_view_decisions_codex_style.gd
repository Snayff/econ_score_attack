...
func _create_decision_entry(decision: Dictionary) -> PanelContainer:
	...
	# Time (could be enhanced with actual turn info)
	var lbl_time: Label = entry.get_node("VBoxContainer/HBoxContainer/LblTime")
	lbl_time.text = "Last Turn"

	# Summary
	var lbl_summary: Label = entry.get_node("VBoxContainer/LblSummary")
	var summary: String = decision["reasoning"]
